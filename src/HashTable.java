class HashTable {
    ...
    // Adicione um contador para saber quantas palavras únicas existem
    ...
    public int Mhash(int v) {
        double A = 0.6180339887; // Constante sugerida por Knuth (Razão Áurea)
        double val = (v & 0x7fffffff) * A;
        double fracionaria = val - Math.floor(val);
        return (int) (size * fracionaria);
    }
    public int Mhash(String chave) { return Mhash(toInt(chave)); }

    // **********************************************************
    // INSERÇÃO COM TRATAMENTO DE COLISÃO
    // **********************************************************
    public void inserir(String chave) {
        int indice = Dhash(chave); // Troque para Mhash quando quiser testar a outra
        Data atual = hashTable[indice];

        // Se o bucket estiver vazio, insere o primeiro
        if (atual == null) {
            hashTable[indice] = new Data(chave, 1);
            totalElementos++;
            return;
        }
        ...
        while (atual != null) {
            if (atual.key.equals(chave)) {
                atual.valor++; // Palavra já existe, aumenta a frequência
                return;
            }
            if (atual.prox == null) break; // Chegou no último nó
            atual = atual.prox;
        }

        // Se saiu do while, a palavra não existe e houve colisão. Adiciona no final.
        atual.prox = new Data(chave, 1);
        totalElementos++;
    }
    ...
    // BUSCAR FREQUÊNCIA
    // **********************************************************
    public int buscarFrequencia(String chave) {
        int indice = Dhash(chave); // Lembre-se de manter igual ao usado na inserção
        Data atual = hashTable[indice];

        while (atual != null) {
            if (atual.key.equals(chave)) return atual.valor;
            atual = atual.prox;
        }
        return 0; // Não achou
    }
    ...
    // **********************************************************
    // OBTER TODAS AS CHAVES (Para o cálculo de Similaridade)
    // **********************************************************
    public String[] obterTodasAsChaves() {
        String[] chaves = new String[totalElementos];
        int index = 0;
        for (int i = 0; i < size; i++) {
            Data atual = hashTable[i];
            while (atual != null) {
                chaves[index++] = atual.key;
                atual = atual.prox;
            }
        }
        return chaves;
    ...
    public int getTamanho() { return totalElementos; }
}